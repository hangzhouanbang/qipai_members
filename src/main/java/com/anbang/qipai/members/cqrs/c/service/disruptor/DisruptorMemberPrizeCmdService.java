...
public class DisruptorMemberPrizeCmdService extends DisruptorCmdServiceBase implements MemberPrizeCmdService, ApplicationListener {
    ...
    public SignPrizeOpportunityValueObject addSignPrizeOpportunity(String memberId, Integer continuousSignDays, Long signTime, VIPEnum vipLevel) {
        ...
        DeferredResult<SignPrizeOpportunityValueObject> result = publishEvent(this.disruptorFactory.getCoreCmdDisruptor(), cmd, new Callable<SignPrizeOpportunityValueObject>() {
            ...
            public SignPrizeOpportunityValueObject call() throws Exception {
                return prizeCmdServiceImpl.addSignPrizeOpportunity(cmd.getParameter(), cmd.getParameter(), cmd.getParameter(), vipLevel);
            }
        });
        try {
    ...
    public SigningRaffleOpportunity addSignRaffleOpportunity(String memberId, Integer continuousSignDays, Long signTime) {
        ...
        DeferredResult<SigningRaffleOpportunity> deferredResult = publishEvent(this.disruptorFactory.getCoreCmdDisruptor(), cmd, new Callable<SigningRaffleOpportunity>() {
            ...
            public SigningRaffleOpportunity call() throws Exception {
                return prizeCmdServiceImpl.addSignRaffleOpportunity(cmd.getParameter(), cmd.getParameter(), cmd.getParameter());
            }
        });
        try {
    ...
    public boolean isRaffleTableInitalized() {
        ...
        DeferredResult<Boolean> deferredResult = publishEvent(this.disruptorFactory.getCoreCmdDisruptor(), cmd, new Callable<Boolean>() {
            @Override
    ...
    public void inializeRaffleTable(List<Lottery> lotteryList) {
        CommonCommand cmd = new CommonCommand(MemberPrizeCmdServiceImpl.class.getName(), "inializeRaffleTable", lotteryList);
        DeferredResult<Object> deferredResult = publishEvent(this.disruptorFactory.getCoreCmdDisruptor(), cmd, new Callable<Object>() {
    ...
    public void onApplicationEvent(ApplicationEvent event) {
        if (event instanceof SignEvent) {
            ...
            this.addSignRaffleOpportunity(signHistoryValueObject.getMemberId(), signHistoryValueObject.getContinuousSignDays(), signHistoryValueObject.getTime());
            this.addSignPrizeOpportunity(signHistoryValueObject.getMemberId(), signHistoryValueObject.getContinuousSignDays(), signHistoryValueObject.getTime(), VIPEnum.of(signHistoryValueObject.getVipLevel()));
        }
    }
